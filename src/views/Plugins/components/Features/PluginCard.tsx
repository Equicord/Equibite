...
export function PluginCard({
    ...
    return (
        <div class="w-full p-6 flex flex-col gap-1 bg-neutral-900 rounded-2xl transition-transform duration-200 hover:outline-2 hover:bg-neutral-800 hover:outline-neutral-700 hover:-translate-y-0.5">
            <h3 class="text-lg text-neutral-300 font-bold">{name}</h3>
            ...
            <Show when={hasCommands && commands.length > 0}>
                <div class="relative group text-sm text-neutral-400 font-medium">
                    <span class="cursor-pointer">Hover to view commands</span>
                    <ul class="absolute left-0 bg-neutral-800 border border-neutral-700 rounded-lg shadow-lg p-4 w-full md:min-w-[400px] hidden group-hover:grid grid-cols-1 md:grid-cols-2 gap-2 z-10">
                        <For each={commands}>
                            {(command) => (
